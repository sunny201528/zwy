...
public class CDEntryHead extends BaseDomain {
	...
	public void setSendName(String sendName) {
		if (sendName != null) {
			...
			sendName = sendName.length() > 50 ? sendName.substring(0, 49)
					: sendName;
		}
	...
	public void setReceiveName(String receiveName) {
		if (receiveName != null) {
			...
			receiveName = receiveName.length() > 50 ? receiveName.substring(0,
					49) : receiveName;
		}
	...
	public String toString() {
		// 2个20131115 3039 ？？
		return "HN||||" + cdHead.getVoyageNO() + "||" + cdHead.getBillNO()
				+ "||" + assBillNO + "||" + cdHead.getI_E_Flag() + "||"
				+ cdHead.getTrafMode() + "||" + cdHead.getI_E_Port() + "||"
				+ sendCountry + "||" + cdHead.getVoyageName() + "||" + sendName
				+ "||" + receiveName + "||" + sendCountry + "||" + sendCity
				+ "||||||||" + cdGName_cn() + "||" + packNO + "||"
				+ NumberUtil.df.format(gGrossWt()) + "||"
				+ NumberUtil.df.format(cdDecl()) + "||" + currCode + "||"
				+ DateUtils.SIMPLE_TIME_DF.format(cdHead.getI_E_Date()) + "||"
				+ DateUtils.SIMPLE_TIME_DF.format(cdHead.getImportTime())
				+ "||" + entryType + "||||||||||||||"
				+ Constants.ENTRY_TYPE.get(entryType + "") + "||" + tradeCO
				+ "||" + tradeName;
	}

	public String toCDString() {
		// 2个20131115 3039 ？？
		return assBillNO + "||" + gName_en + "||" + cdGName_cn() + "||"
				+ packNO + "||" + NumberUtil.df.format(gGrossWt()) + "||"
				+ NumberUtil.df.format(cdDecl()) + "||" + currCode + "||"
				+ sendName + "||" + receiveName + "||" + sendCountry + "||"
				+ sendCity + "||||" + entryType;
	}
	...
	private Double cdDecl() {
		String[] tmp = countDeclTotal().split("\\|");
		Double total = 0d;
	...
	public List<CDEntryListVo> getEntryList() {
		...
		// 处理数量
		String[] gGrossWts = null;
		if (this.gGrossWt != null) {
			gGrossWts = gGrossWt.split("\\|");
		}
		String[] declTotals = null;
		if (this.countDeclTotal() != null) {
			declTotals = countDeclTotal().split("\\|");
		}
		String[] gModels = null;
		if (this.gModel != null) {
			gModels = gModel.split("\\|");
		}
		if (names == null || codes == null || gGrossWts == null
				|| declTotals == null || gModels == null) {
			return entryList;
		}
		if (names.length != codes.length || names.length != declTotals.length
				|| names.length != gGrossWts.length
				|| names.length != gModels.length) {
			System.out.println(codeTS);
			System.out.println(gName_cn);
			System.out.println(countDeclTotal());
		...
		try {
			...
			for (int i = 0; i < codes.length; i++) {
				...
				entry.setgGrossWt(Double.valueOf(gGrossWts[i]));
	...
	// wt[i] = tmp;
	// } else {
	// wt[i] = new BigDecimal(oneDecl * gGrossWt / totalDecl)
	...
	// 设置品名
	private void setProductName(String gName_cn) {
		String[] names = null;
		if (gName_cn != null) {
			ProductName productName = null;
			names = gName_cn.split("\\|");
			for (int i = 0; i < names.length; i++) {
				productName = ProductNameCache.get(names[i]);
				if (productName == null) {
					throw new RuntimeException(gName_cn + ":对应的品名信息不存在");
				}
				if (i == 0) {
					codeTS = productName.getCodeTS();
					gModel = productName.getgModel();
					gQty = "1";
				} else {
					codeTS += "|" + productName.getCodeTS();
					gModel += "|" + productName.getgModel();
	...
	private String countDeclTotal() {

		String[] names = gName_cn.split("\\|");
		String[] gGrossWtString = gGrossWt.split("\\|");
		List<Double> wts = new ArrayList<Double>();
		List<Double> total = new ArrayList<Double>();// 申报总价格
		Double tmpTotal = 0d;
		String result = "";
		for (int i = 0; i < gGrossWtString.length; i++) {
			ProductName productName = ProductNameCache.get(names[i]);
			Double p = Double.valueOf(gGrossWtString[i]);
			wts.add(p);
			Double tmp = p * productName.getPrice();
			total.add(tmp);
			tmpTotal += tmp;
			if (i == 0) {
				result = NumberUtil.df.format(tmp);
			} else {
				result += "|" + NumberUtil.df.format(tmp);
			}
		}
		...
		if (tmpTotal <= 270) {
			for (int i = 0; i < total.size(); i++) {
				if (i == 0) {
					result = NumberUtil.df
							.format(total.get(i) / tmpTotal * 280);
				} else {
					result += "|"
							+ NumberUtil.df.format(total.get(i) / tmpTotal
									* 280);
				}
			}
		}
